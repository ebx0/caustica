...
Written by `scripts/run_gpu_local.sh` on 2026-09-06T20:43:45Z (UTC).
...
|---|---|---|---|
| gpu tests | `python -m pytest -m "gpu and not kwave and not network"` | 0 | 8 passed, 966 deselected, 1 warning in 2.05s |
| gate suite | `python -m caustica.validation gpu-gates` | 4 | overall: FAIL |
...
- FAIL        time: post-calibration wall-time prediction within +/-25% of actual, on at least 2 scenarios (same device); a plan whose source is not 'calibrated' does not count
