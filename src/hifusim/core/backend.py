...
* Solvers and other heavy numerical code never import numpy/cupy at module
  ...
  decision that keeps hifusim free of precompiled binaries (k-Wave's Colab
  failure mode) and usable on Windows (j-Wave/JAX's weak spot).
* The cupy import is *lazy*: importing hifusim on a GPU-less machine never
    ...
    if name == "cupy":
        if not cupy_available():
            raise RuntimeError(
                ...
                "Install the extra (pip install hifusim[gpu]) on a CUDA machine, "
