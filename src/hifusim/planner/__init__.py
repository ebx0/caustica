...
* ``"calibrated"`` — ``t_step = a*P*log2(P) + b*P`` coefficients fitted by
  :func:`hifusim.planner.calibrate` (~20 real steps on the device, persisted
  in ``~/.hifusim/calibration.json``). The ±25% accuracy gate applies here.
...
(:mod:`hifusim.planner.model`); when it does not fit, ``advice`` carries
...
from hifusim.planner.calibration import (
    calibrate,
    default_calibration_path,
    find_calibration_for,
    measure_step_time,
)
from hifusim.solvers.base import CWRunSpec
from hifusim.solvers.kspace.engine import (
    cw_discretization,
    cw_tof_periods,
...
class GPUSpec:
    ...
    def usable_bytes(self) -> int:
        """VRAM available to hifusim: capacity minus CUDA context/runtime."""
...
def load_gpu_db() -> tuple[dict[str, GPUSpec], dict[str, str]]:
    ...
    raw = json.loads(resources.files("hifusim.planner").joinpath("gpu_db.json").read_text())
    devices = {
