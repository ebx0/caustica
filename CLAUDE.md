...
The documentation repository (`ebx0/ebx0.github.io`, branch
`caustica-docs`) is pushed by the same session under the same rules, with
the user's permission of 2026-09-06, once the first parity battery has run
locally. It is a separate MkDocs site and is not cloned inside this tree;
clone it beside the library, never within it, so a `git add` in one can
never reach the other. Its pages are generated by its own `scripts/make_*.py`
from the library's measurement files: a number edited on the site but not in
the library is a number with no measurement behind it, so the generator is
